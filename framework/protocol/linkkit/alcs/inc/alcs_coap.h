...
//会自动生成msgid & token
void alcs_msg_init(CoAPContext *ctx, CoAPMessage *message, int code, unsigned char type, int keep, CoAPLenString* payload, void *userdata);
...
//observe: other: 没意义  
int alcs_sendrsp(CoAPContext *ctx, NetworkAddr* addr, CoAPMessage *message, char observe, unsigned short msgid, CoAPLenString* token);
