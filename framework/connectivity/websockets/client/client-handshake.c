...
lws_client_connect_2(struct lws *wsi)
{
    ...
    int rec1=connect(wsi->desc.sockfd, (const struct sockaddr *)&sa46, n) ;
	if (rec1== -1 ||
	    LWS_ERRNO == LWS_EISCONN) {
       // lwsl_notice("===Connect failed rec=%d errno=%d wsi->desc.sockfd=%d LWS_EISCONN=%d\n",rec1, LWS_ERRNO,wsi->desc.sockfd,LWS_EISCONN);
