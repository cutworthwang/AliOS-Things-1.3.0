...
void NetCallback(hal_wifi_ip_stat_t *pnet)
{
    if (sim_aos_wifi_stm23l496.ev_cb == NULL)
        return;
    if (sim_aos_wifi_stm23l496.ev_cb->ip_got == NULL)
        return;

    sim_aos_wifi_stm23l496.ev_cb->ip_got(&sim_aos_wifi_stm23l496, pnet, NULL);
}
...
void connected_ap_info(hal_wifi_ap_info_adv_t *ap_info, char *key, int key_len)
{
    if (sim_aos_wifi_stm23l496.ev_cb == NULL)
        return;
    if (sim_aos_wifi_stm23l496.ev_cb->para_chg == NULL)
        return;

    sim_aos_wifi_stm23l496.ev_cb->para_chg(&sim_aos_wifi_stm23l496, ap_info, key, key_len, NULL);
}
...
void WifiStatusHandler(int status)
{
    if (sim_aos_wifi_stm23l496.ev_cb == NULL)
        return;
    if (sim_aos_wifi_stm23l496.ev_cb->stat_chg == NULL)
        return;

    sim_aos_wifi_stm23l496.ev_cb->stat_chg(&sim_aos_wifi_stm23l496, (hal_wifi_event_t)status, NULL);
}
...
void ApListCallback(hal_wifi_scan_result_t *pApList)
{
    ...
    if (sim_aos_wifi_stm23l496.ev_cb == NULL)
        return;
    if (sim_aos_wifi_stm23l496.ev_cb->scan_compeleted == NULL)
        return;

    sim_aos_wifi_stm23l496.ev_cb->scan_compeleted(&sim_aos_wifi_stm23l496, 
...
void ApListAdvCallback(hal_wifi_scan_result_adv_t *pApAdvList)
{
    if (sim_aos_wifi_stm23l496.ev_cb == NULL)
        return;
    if (sim_aos_wifi_stm23l496.ev_cb->scan_adv_compeleted == NULL)
        return;

    sim_aos_wifi_stm23l496.ev_cb->scan_adv_compeleted(&sim_aos_wifi_stm23l496, 
        pApAdvList, NULL);
}

hal_wifi_module_t sim_aos_wifi_stm23l475 = {
    .base.name           = "sim_aos_wifi_stm23l496",
    .init                =  wifi_init,
