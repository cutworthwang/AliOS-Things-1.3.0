 ...
 *     - ESP_FAIL Parameter error
 */
esp_err_t uart_driver_install(uart_port_t uart_num, int rx_buffer_size, int tx_buffer_size, int queue_size, espos_queue_t *uart_queue, int intr_alloc_flags);
 ...
 */
esp_err_t uart_wait_tx_done(uart_port_t uart_num, espos_tick_t ticks_to_wait);
 ...
 */
int uart_read_bytes(uart_port_t uart_num, uint8_t* buf, uint32_t length, espos_tick_t ticks_to_wait);
