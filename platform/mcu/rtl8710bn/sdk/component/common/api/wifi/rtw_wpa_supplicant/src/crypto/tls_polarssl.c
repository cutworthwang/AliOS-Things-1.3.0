...
void * tls_init(const struct tls_config *conf)
{
	struct mbedtls_ssl *mbed_ssl;

	mbedtls_platform_set_calloc_free(my_calloc, vPortFree);  

	mbed_ssl = os_zalloc(sizeof(*mbed_ssl));   

	mbed_ssl->ssl = os_zalloc(sizeof(mbedtls_ssl_context));
	mbed_ssl->conf = os_zalloc(sizeof(mbedtls_ssl_config));  
	mbed_ssl->fd = os_zalloc(sizeof(mbedtls_net_context));       

	if((mbed_ssl == NULL)||(mbed_ssl->ssl == NULL)||(mbed_ssl->conf == NULL)||(mbed_ssl->fd == NULL))
		return NULL;

	mbedtls_net_init(mbed_ssl->fd);
	mbedtls_ssl_init(mbed_ssl->ssl);
	mbedtls_ssl_config_init(mbed_ssl->conf);
...
{
	if(ssl_ctx != NULL){
		mbedtls_net_free((( struct mbedtls_ssl *)ssl_ctx)->fd);		
		mbedtls_ssl_free((( struct mbedtls_ssl *)ssl_ctx)->ssl);
		mbedtls_ssl_config_free((( struct mbedtls_ssl *)ssl_ctx)->conf);	
		os_free(ssl_ctx, 0);
		ssl_ctx = NULL;
	}
}
...
{
	int ret;
	struct mbedtls_ssl *ssl_context = tls_ctx;
	...
	//test_BIO(conn);
 	mbedtls_ssl_set_bio(ssl_context->ssl, ssl_context->fd, buf_write, buf_read, NULL);       
        
	if((ret = mbedtls_ssl_config_defaults(ssl_context->conf,
	...
	mbedtls_ssl_conf_authmode(ssl_context->conf, MBEDTLS_SSL_VERIFY_NONE);
	mbedtls_ssl_conf_rng(ssl_context->conf, my_random, NULL);

	if((ret = mbedtls_ssl_setup(ssl_context->ssl, ssl_context->conf)) != 0) {
...
int tls_connection_established(void *tls_ctx, struct tls_connection *conn)
{
	mbedtls_ssl_context *ssl = ((struct mbedtls_ssl *)tls_ctx)->ssl;
				       ...
				       const struct wpabuf *in_data)
{
	...
	struct mbedtls_ssl *mbed_tls_ctx = tls_ctx;        
	mbedtls_ssl_context *ssl = mbed_tls_ctx->ssl;
	struct wpabuf * out_data;
	int size;
	int res;
