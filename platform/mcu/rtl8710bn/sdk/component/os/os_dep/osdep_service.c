...
void rtw_enter_critical(_lock *plock, _irqL *pirqL)
{
	rtw_enter_critical_bh( plock,  pirqL);
}

void rtw_exit_critical(_lock *plock, _irqL *pirqL)
{
	rtw_exit_critical_bh( plock,  pirqL);
}
							   ...
							   osdepTickType xBlockTime )
{
	if(osdep_service.rtw_timerChangePeriod){
		if(osdep_service.rtw_timerChangePeriod(xTimer, xNewPeriod, xBlockTime))
			return rtw_timerStart(xTimer,xBlockTime);
	    }
	else
