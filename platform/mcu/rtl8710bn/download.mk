...
download_app: $(STRIPPED_LINK_OUTPUT_FILE) display_map_summary kill_openocd
	...
	$(call CONV_SLASHES, $(OPENOCD_FULL_NAME)) -s $(SOURCE_ROOT) -f $(OPENOCD_CFG_PATH)interface/$(JTAG).cfg -f $(OPENOCD_CFG_PATH)rtl8710/rtl8710.cfg -c "gdb_port disabled" -c "tcl_port disabled" -c "telnet_port disabled" -c init -c erase_chip -c "program_flash $(OUTPUT_DIR)/binary/boot_all.bin 0" -c "program_flash $(SOURCE_ROOT)/board/amebaz_dev/system.bin 0x9000" -c "program_flash $(OUTPUT_DIR)/binary/image2_all_ota1.bin 0xB000" -c reset -c shutdown $(DOWNLOAD_LOG) 2>&1 && $(ECHO) Download complete && $(ECHO_BLANK_LINE) || $(ECHO) Download failed. See $(OPENOCD_LOG_FILE) for detail.
