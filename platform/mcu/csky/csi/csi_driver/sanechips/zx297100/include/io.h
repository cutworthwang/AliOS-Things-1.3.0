...
#include <stdint.h>
//#include <bsp/yunos_bsp_spinlock.h>
#define BIT(n)			(1UL << (n))
...
#if 0
static inline void putreg32_lock(uint32_t val, volatile uint32_t *addr)
{
    hwspinlock_reg_lock();
	putreg32(val,addr);
    hwspinlock_reg_unlock();  
}
#endif
...
static inline void __set_bit(int nr, volatile uint32_t *addr)
{
	*addr |= BIT(nr);
}

static inline void __clear_bit(int nr, volatile uint32_t *addr)
{
	*addr &= ~BIT(nr);
}

static inline void __write_bit(int nr, volatile uint32_t *addr, int val)
{
	uint32_t temp = *addr;
...
static inline void __write_bits(int width, int nr, volatile uint32_t *addr, int val)
{
	uint32_t temp = *addr;
...
#if 0
static inline void __write_bit_lock(int nr, volatile uint32_t *addr, int val)
{
    hwspinlock_reg_lock();
	__write_bit(nr, addr, val);
    hwspinlock_reg_unlock(); 

}

static inline void __write_bits_lock(int width, int nr, volatile uint32_t *addr, int val)
{
    hwspinlock_reg_lock();
	__write_bits(width, nr, addr, val);
    hwspinlock_reg_unlock();  
}
