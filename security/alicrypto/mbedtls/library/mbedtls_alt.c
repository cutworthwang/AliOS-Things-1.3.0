...
__attribute__((__used__)) void OSA_mutex_init(mbedtls_threading_mutex_t *mutex)
{
...
__attribute__((__used__)) void OSA_mutex_free(mbedtls_threading_mutex_t *mutex)
{
    if (mutex == NULL || !mutex->is_valid) {
        return;
    }
...
__attribute__((__used__)) int OSA_mutex_lock(mbedtls_threading_mutex_t *mutex)
{
    if (mutex == NULL || !mutex->is_valid) {
...
__attribute__((__used__)) int OSA_mutex_unlock( mbedtls_threading_mutex_t *mutex )
{
    if( mutex == NULL || ! mutex->is_valid ) {
        return( MBEDTLS_ERR_THREADING_BAD_INPUT_DATA );
    }
