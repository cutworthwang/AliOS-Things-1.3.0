...
static void mgf_mask( unsigned char *dst, size_t dlen, unsigned char *src,
                      size_t slen, mbedtls_hash_context_t *md_ctx )
{
    ...
    hlen = mbedtls_hash_get_size( md_ctx->md_info );
    ...
    while( dlen > 0 )
    {
        ...
        mbedtls_hash_starts( md_ctx );
        mbedtls_hash_update( md_ctx, src, slen );
        mbedtls_hash_update( md_ctx, counter, 4 );
        mbedtls_hash_finish( md_ctx, mask );
                            ...
                            unsigned char *output )
{
    ...
    md_info = mbedtls_hash_info_from_type( (mbedtls_md_type_t) ctx->hash_id );
    if( md_info == NULL )
        return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );

    olen = ctx->len;
    hlen = mbedtls_hash_get_size( md_info );
    ...
    *p++ = 0;
    ...
    /* Construct DB */
    mbedtls_hash( md_info, label, label_len, p );
    p += hlen;
    p += olen - 2 * hlen - 2 - ilen;
    *p++ = 1;
    memcpy( p, input, ilen );

    mbedtls_hash_init( &md_ctx );
    if( ( ret = mbedtls_hash_setup( &md_ctx, md_info, 0 ) ) != 0 )
    {
        mbedtls_hash_free( &md_ctx );
        return( ret );
    }
                            ...
                            size_t output_max_len )
{
    ...
    /* Generate lHash */
    mbedtls_hash( md_info, label, label_len, lhash );
                         ...
                         unsigned char *sig )
{
    ...
    if( md_alg != MBEDTLS_MD_NONE )
    {
        ...
        md_info = mbedtls_hash_info_from_type( md_alg );
        if( md_info == NULL )
            return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );

        hashlen = mbedtls_hash_get_size( md_info );
    }
    ...
    *p++ = 0x01;
    ...
    /* Generate H = Hash( M' ) */
    mbedtls_hash_starts( &md_ctx );
    mbedtls_hash_update( &md_ctx, p, 8 );
    mbedtls_hash_update( &md_ctx, hash, hashlen );
    mbedtls_hash_update( &md_ctx, salt, slen );
    mbedtls_hash_finish( &md_ctx, p );
                               ...
                               unsigned char *sig )
{
    ...
    if( md_alg != MBEDTLS_MD_NONE )
    {
        const mbedtls_md_info_t *md_info = mbedtls_hash_info_from_type( md_alg );
        if( md_info == NULL )
            return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );
                               ...
                               const unsigned char *sig )
{
    ...
    md_info = mbedtls_hash_info_from_type( mgf1_hash_id );
    if( md_info == NULL )
        return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );
     ...
     */
    mbedtls_hash_starts( &md_ctx );
    mbedtls_hash_update( &md_ctx, zeros, 8 );
    mbedtls_hash_update( &md_ctx, hash, hashlen );
    mbedtls_hash_update( &md_ctx, p, slen );
    mbedtls_hash_finish( &md_ctx, result );
