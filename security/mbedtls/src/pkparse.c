...
static unsigned char CRT_OID_PKCS1_RSA[] = {
                             ...
                             mbedtls_rsa_context *rsa )
{
    ...
    *p += len;
    ...
    MBEDTLS_ALT_DEBUG_BUF(3, "rsa_n", rsa->rsa_n, rsa->n_len);
    MBEDTLS_ALT_DEBUG_BUF(3, "rsa_e", rsa->rsa_e, rsa->e_len);
...
#if defined(MBEDTLS_IOT_SPECIFIC)
    if( alg_oid.len != CRT_OID_IDENT_LEN ||
        memcmp(alg_oid.p, CRT_OID_PKCS1_RSA, CRT_OID_IDENT_LEN ) ) {
        MBEDTLS_ALT_DEBUG_BUF(1, "pk_alg_oid", alg_oid.p, alg_oid.len);
