...
static unsigned char CRT_OID_PKCS1_SHA1[] = {
    0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x05
};
static unsigned char CRT_OID_PKCS1_SHA256[] = {
                      ...
                      void **sig_opts )
{
    ...
    *pk_alg = MBEDTLS_PK_NONE;
    ...
    if( sig_oid->len == CRT_OID_IDENT_LEN) {
#if defined(MBEDTLS_MD5_C)
        if (!memcmp(sig_oid->p, CRT_OID_PKCS1_MD5, CRT_OID_IDENT_LEN)) {
        ...
        if (!memcmp(sig_oid->p, CRT_OID_PKCS1_SHA1, CRT_OID_IDENT_LEN)) {
            *md_alg = MBEDTLS_MD_SHA1;
            *pk_alg = MBEDTLS_PK_RSA;
        }
        ...
        if (!memcmp(sig_oid->p, CRT_OID_PKCS1_SHA256, CRT_OID_IDENT_LEN)) {
    ...
    if (*md_alg == MBEDTLS_MD_NONE) {
        MBEDTLS_ALT_DEBUG_BUF(2, "x509_sig_oid", sig_oid->p, sig_oid->len);
        return( MBEDTLS_ERR_X509_UNKNOWN_SIG_ALG + MBEDTLS_ERR_OID_NOT_FOUND );
    }
