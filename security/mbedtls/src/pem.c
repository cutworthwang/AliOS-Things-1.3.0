...
#else
    mbedtls_aes_init_alt( &aes_ctx );

    pem_pbkdf1( aes_key, keylen, aes_iv, pwd, pwdlen );

    mbedtls_aes_setkey_dec_alt( &aes_ctx, aes_key, keylen * 8 );
    mbedtls_aes_crypt_cbc_alt( &aes_ctx, MBEDTLS_AES_DECRYPT, buflen,
                     aes_iv, buf, buf );

    mbedtls_aes_free_alt( &aes_ctx );
    mbedtls_zeroize( aes_key, keylen );
#endif
}
#endif /* MBEDTLS_AES_C */
